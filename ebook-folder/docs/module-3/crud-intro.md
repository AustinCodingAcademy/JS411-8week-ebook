...
Great, so we now have an idea what a CRUD application is, so let's get started. In order for us to interact with a Database, we first need to create it. We should already have a project created on Firebase from our [Authentication with Firebase lesson](../module-2/implement-fireBase.md.md), so let's continue to work on that same one.
