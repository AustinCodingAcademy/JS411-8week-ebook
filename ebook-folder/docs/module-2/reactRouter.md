...
The (#5) step is to use the "wrap". To do this we'll pass to `root.render()` the `Main` component we just built. Now our app's `index.js` file should look like this:
...
3. Remember to import your custom Router component, `import Router from './Router'` so it can be used in the index.js file.
